...
/* 38. Write a solution to swap the seat id of every two consecutive
students. If the number of students is odd,the id of the last student
is not swapped. Return result table ordered by id in ascending
order.*/
...
/* 41. Write a solution to find the people who have most friends and
the most friends number.The test cases are generated so that only one person
...
/* 42. Write a solution to report the sum of all total investment values in 2016 tiv_2016, for 
all policyholders who: have same tiv_2015 value as one or more other policyholders, and are not
located in same city as any other policyholder(i.e (lat,lon) attribute pairs must be unique).
Round tiv_2016 to two decimal places.*/
WITH CheckedRows AS(
  SELECT tiv_2016, COUNT(*) OVER (PARTITION BY tiv_2015) AS tiv_2015_count,COUNT(*) OVER
  (PARTITIOB BY lat,lon)
  AS loc_count
  FROM Insurance
  )
